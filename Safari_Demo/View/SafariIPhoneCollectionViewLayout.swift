...
class SafariIPhoneCollectionViewLayout: UICollectionViewLayout {
    ...
    public var itemHeight: CGFloat = kScreenH
    ...
    override public func prepare() {
        ...
        contentSize = CGSize(width: width, height: itemHeight)
        ...
        if attributes.count > 0, let lastItemAttributes = attributes.last {
            let newHeight = lastItemAttributes.frame.origin.y + lastItemAttributes.frame.size.height + bottomItemPadding
    ...
    private func makePerspectiveTranslation(outOf angleOfRotation: CGFloat, using scale: CGPoint) -> CATransform3D {
        let angle = (CGFloat.pi * angleOfRotation / 180.0)
        let rotation = CATransform3DMakeRotation(angle, 1.0, 0.0, 0.0)
        let depth = CGFloat(300.0)
        
        let translateDown = CATransform3DMakeTranslation(0.0, -60, -depth)
